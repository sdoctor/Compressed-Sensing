...
clc                                                       
%% Clearing up command window and workspace
clear
clear all;
close all;

for count=1:1
%eval(['I=double(256*rgb2gray(imread(''E:\comprsensing\2Ddatabase\1_' num2str(count) '_s.bmp'')));']);                       
%% Image Intake                       %% Image Intake
%I =I(1:212,1:212);

I=double(imread('cameraman.tif'));

s=size(I);                                               
%% Storing dimensions of image
...
for i=1:5
 ...
 eval(['[MeasFrac' num2str(i) ',SNR' num2str(i) ',MeasFrac' num2str(i+5) ',SNR' num2str(i+5) ',MeasFrac' num2str(i+10) ',SNR' num2str(i+10),MeasFrac' num2str(i+15) ',SNR' num2str(i+15) ']= cod(I,Transformrow' num2str(i) ',Transformapp' num2str(i) ',str' num2str(i) ',' num2str(i) ' );']);
end
...
figure;
plot(MeasFrac1,SNR1avg,'*',MeasFrac2,SNR2avg,'*',MeasFrac3,SNR3avg,'*',MeasFrac4,SNR4avg,'*',MeasFrac5,SNR5avg,'*',MeasFrac6,SNR6avg,'*',MeasFrac7,SNR7avg,'*',MeasFrac8,SNR8avg,'*',MeasFrac9,SNR9avg,'*',MeasFrac10,SNR10avg,'*',MeasFrac11,SNR11avg,'*',MeasFrac12,SNR12avg,'*',MeasFrac13,SNR13avg,'*',MeasFrac14,SNR14avg,'*',MeasFrac15,SNR15avg,'*');                               
%% Plot Aerr: Error fraction vs measurements used
axis([0 1 20 60]);
